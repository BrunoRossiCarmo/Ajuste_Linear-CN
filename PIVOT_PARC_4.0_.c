...
//------------Realizar-Processo-----------------//
void do_it(float matriz[][9], int linha, int coluna, float pivo, int inicio){  //---------------->Essa função realiza a troca de termos nas linhas utilizando método do pivotamento parcial
	int cont = 1;
	int cont2 = 0;
	int pass;      //-------------> Armazenador
	float constant;  //-------------> constante = aji/pivo
	while(cont<linha){
		if(matriz[cont][coluna]!=0){
			...
			cont2 = 0;
				while (cont2 < 9){
					matriz[pass][cont2] =  (matriz[pass][cont2]) - (constant*(matriz[inicio][cont2]));                   //Ln = Ln - constante*(L(n-1))
					cont2++;
				}			
		}
		cont++;
